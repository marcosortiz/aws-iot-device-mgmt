...
def createkeysandcertificate_execution_accepted(response):
    ...
    try:
        ...
        logger.info("Received a new message {}".format(createKeysAndCertificateResponse))
        time.sleep(2)
...
def createcertificatefromcsr_execution_accepted(response):
    ...
    try:
        ...
        logger.info("Received a new message {}".format(createCertificateFromCsrResponse))
        time.sleep(2)
...
def registerthing_execution_accepted(response):
    ...
    try:
        ...
        logger.info("wrote files: cert_file_name: {} key_file_name: {}".format(cert_file_name, key_file_name))
...
def waitForCreateKeysAndCertificateResponse():
    ...
    while loopCount < 10 and createKeysAndCertificateResponse is None:
        if createKeysAndCertificateResponse is not None:
            break
        logger.info('Waiting... CreateKeysAndCertificateResponse: ' + json.dumps(createKeysAndCertificateResponse))
        loopCount += 1
        time.sleep(1)
...
def waitForCreateCertificateFromCsrResponse():
    ...
    while loopCount < 10 and createCertificateFromCsrResponse is None:
        if createCertificateFromCsrResponse is not None:
            break
        logger.info('Waiting...CreateCertificateFromCsrResponse: ' + json.dumps(createCertificateFromCsrResponse))
        loopCount += 1
        time.sleep(1)
...
def waitForRegisterThingResponse():
    ...
    while loopCount < 20 and registerThingResponse is None:
        if registerThingResponse is not None:
            break
        loopCount += 1
        logger.info('Waiting... RegisterThingResponse: ' + json.dumps(registerThingResponse))
        time.sleep(1)
